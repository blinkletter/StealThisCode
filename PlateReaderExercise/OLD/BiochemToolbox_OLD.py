...
def plot_wells(data_file_path, plate_name_list, Column_list, Row_list,    #DEPRECATED
...
def make_plate_data_for_setup(file_name):  #DEPRECATED
...
def make_data_files_plates(setupdata, outfile_rootname, pH = 7.0):   #DEPRECATED
    ...
    for lane_name, E_name, Vmax_value, KM_value in parameters:
...
def collect_lanes(data_file_path, plate_name_list, plate_plan_path,    #DEPRECATED
                Fraction_time_span = 1, 
                pH = 7,
...
def make_data(setup_file_name, outfile_rootname, pH = 7.0):
   ...
   # df.dropna(axis = 0, inplace = True)   # remove any rows with NaN values
    ...
    setupdata = pd.DataFrame.from_dict(dictionary)

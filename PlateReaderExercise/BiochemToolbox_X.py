...
def make_plate_data_for_setup(file_name):
   ...
   # df.dropna(axis = 0, inplace = True)   # remove any rows with NaN values
    ...
    dictionary["S_conc"] = df["S_Conc"]         ### in millimolar
    dictionary["lane_name"] = df["Column"]
    dictionary["E_conc"] = df["E_Conc"]         ### in nanomolar
    dictionary["kcat"] = df["kcat"]             ### in s^-1
    dictionary["Vmax"] = df["kcat"]*60 * df["E_Conc"]/1E9 * 1E6   ### Vmax from kcat and [E] in micromolar/min       
    dictionary["KM"] = df["KM"]                 ### in millimolar 
...
def make_data_files_plates(setupdata, file_name, pH = 7.0):
    ...
    Expects that...
        Vmax values are in micromolar/min 
        k_cat values are in s^-1
        E_conc values are in nanomolar
        S_conc values are in millimolar
    Outputs...
        time in minutes
        absorbance units
    '''
    ...
    eq, f = get_integrated_MM_function()    # eq is sympy equation object, f is the corresponding function
                                            # function needs Vmax in units of molar/s
    ...
    parameters = zip(setupdata["lane_name"],
                    setupdata["Enzyme"],    # nanomolar
                    setupdata["Vmax"],      # micromolar/min
                    setupdata["KM"])        # millimolar

    for p in parameters:
        lane_name, E_name, Vmax_value, KM_value = p   ### unpack variables

        row_df = setupdata[["row_name","S_conc"]].dropna()
        row_info = zip(row_df["row_name"], row_df["S_conc"])  # S_conc in millimolar

        for row in row_info:
            row_name, S0_value = row      ### unpack row name and substrate conc (millimolar)
            ...
            product = product_E + product_NPA  # Add enzyme and background reaction rates
